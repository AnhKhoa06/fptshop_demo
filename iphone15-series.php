...
if ($sort === 'high_to_low') {
    $sql = "SELECT * FROM products WHERE brand_id = 1 AND prd_name LIKE 'iPhone 15%' ORDER BY price_discount DESC LIMIT 20";
} elseif ($sort === 'low_to_high') {
    $sql = "SELECT * FROM products WHERE brand_id = 1 AND prd_name LIKE 'iPhone 15%' ORDER BY price_discount ASC LIMIT 20";
} elseif ($sort === 'hot_deals') {
    $sql = "SELECT DISTINCT p.* FROM products p 
            INNER JOIN flash_sales fs ON p.prd_id = fs.product_id 
            WHERE p.brand_id = 1 AND p.prd_name LIKE 'iPhone 15%' 
            AND fs.start_time <= NOW() AND fs.end_time >= NOW() 
            ORDER BY p.prd_id ASC LIMIT 20";
} elseif ($sort === 'view_count') {
    $sql = "SELECT * FROM products WHERE brand_id = 1 AND prd_name LIKE 'iPhone 15%' ORDER BY views DESC LIMIT 20";
} else {
    $sql = "SELECT * FROM products WHERE brand_id = 1 AND prd_name LIKE 'iPhone 15%' ORDER BY prd_id ASC LIMIT 20";
}
?>

<title>iPhone 15 Series</title>
...
<div class="breadcrumb">
    ...
    <i class="fas fa-angle-right"></i><a href="brand/apple-iphone.php">Apple (iPhone)</a>
    <i class="fas fa-angle-right"></i>
    <span>iPhone 15 Series</span>
</div>

<h2 class="brand-title">iPhone 15 Series</h2>
    ...
    <div class="product-display">
        <div class="product-list">
            ...
            while ($row = mysqli_fetch_assoc($query)) {
                ...
                mysqli_stmt_bind_param($default_color_stmt, "i", $row['prd_id']);
