...
<div class="breadcrumb">
    ...
    <a href="../index.php">Điện thoại</a>
        <i class="fa-solid fa-angle-right"></i>
    <a href="../brand/xiaomi.php">Xiaomi</a>
        <i class="fa-solid fa-angle-right"></i>
    <span>Xiaomi 13 512GB</span>
</div>


<?php
// Giả định product_id của iPhone 15 là 1
$product_id = 4; // Thay đổi ID này dựa trên sản phẩm thực tế, ví dụ: lấy từ $_GET['id']
...
// Tính tổng số lượng đã bán (sold) từ flash_sales và products
$total_sold = 0;
$current_time = date('Y-m-d H:i:s');

// Lấy sold từ flash_sales (không kiểm tra thời gian start_time và end_time)
$flash_sale_query = "SELECT sold FROM flash_sales WHERE product_id = ? AND color = ? AND rom = ? LIMIT 1";
$flash_sale_stmt = mysqli_prepare($connect, $flash_sale_query);
mysqli_stmt_bind_param($flash_sale_stmt, "iss", $product_id, $selected_color, $selected_rom);
...
if ($flash_sale_row = mysqli_fetch_assoc($flash_sale_result)) {
    $total_sold += $flash_sale_row['sold'];
}
mysqli_stmt_close($flash_sale_stmt);

// Lấy sold từ products
$product_sold_query = "SELECT sold FROM products WHERE prd_id = ?";
$product_sold_stmt = mysqli_prepare($connect, $product_sold_query);
mysqli_stmt_bind_param($product_sold_stmt, "i", $product_id);
mysqli_stmt_execute($product_sold_stmt);
$product_sold_result = mysqli_stmt_get_result($product_sold_stmt);
if ($product_sold_row = mysqli_fetch_assoc($product_sold_result)) {
    $total_sold += $product_sold_row['sold'];
}
mysqli_stmt_close($product_sold_stmt);
...
$brand_id_iphone = 4; // Giả định brand_id của Apple (iPhone) là 1, bạn cần thay đổi theo thực tế
...
<div id="product-container" class="product-detail">
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane" id="reviews">
                    ...
                    if (!$connect) {
                        ...
                        // Truy vấn đánh giá
                        $product_id = 4; // Thay bằng ID sản phẩm thực tế (ví dụ: từ $_GET['product_id'])
    ...
    <div class="product-right">
        ...
        <div class="product-meta">
            ...
            <span class="divider">|</span>
            <a href="">Đã bán: <?= $total_sold > 0 ? number_format($total_sold) : 0; ?></a>
            <span class="divider">|</span>
        ...
        <?php
        // Lấy thông tin sản phẩm hiện tại
        $product_id = isset($_GET['product_id']) ? (int)$_GET['product_id'] : 4;
        ...
        $brand_id = $current_product['brand_id'] ?? 4; // Giả định brand_id của iPhone là 1
