...
if ($sort === 'high_to_low') {
    $sql = "SELECT * FROM products WHERE brand_id = 16 ORDER BY price_discount DESC LIMIT 20";
} elseif ($sort === 'low_to_high') {
    $sql = "SELECT * FROM products WHERE brand_id = 16 ORDER BY price_discount ASC LIMIT 20";
    ...
    $sql = "SELECT * FROM products WHERE brand_id = 16 ORDER BY views DESC LIMIT 20";
} else {
    $sql = "SELECT * FROM products WHERE brand_id = 16 ORDER BY prd_id ASC LIMIT 20";
}
?>
<title>Smart Phone | MacBook</title>
...
<h2 class="brand-title" style="margin-left: 40px;"><i class="fab fa-apple"></i> Macbook</h2>
