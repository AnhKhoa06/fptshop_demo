...
if ($sort === 'high_to_low') {
    $sql = "SELECT * FROM products WHERE brand_id = 4 ORDER BY price_discount DESC LIMIT 20";
} elseif ($sort === 'low_to_high') {
    $sql = "SELECT * FROM products WHERE brand_id = 4 ORDER BY price_discount ASC LIMIT 20";
    ...
    $sql = "SELECT * FROM products WHERE brand_id = 4 ORDER BY views DESC LIMIT 20";
} else {
    $sql = "SELECT * FROM products WHERE brand_id = 4 ORDER BY prd_id ASC LIMIT 20";
}
?>
<title>Smart Phone | Xiaomi</title>
...
<h2 class="brand-title">
    <img src="../img/xiaomi7.png" alt="Xiaomi Logo" class="brand-logo" style="width: 75px; height: 40px; vertical-align: middle; margin-right: -15px; ">
    Xiaomi
</h2>
