...
if ($sort === 'high_to_low') {
    $sql = "SELECT * FROM products WHERE brand_id IN (2, 35) ORDER BY price_discount DESC LIMIT 20";
} elseif ($sort === 'low_to_high') {
    $sql = "SELECT * FROM products WHERE brand_id IN (2, 35) ORDER BY price_discount ASC LIMIT 20";
} elseif ($sort === 'hot_deals') {
    $sql = "SELECT DISTINCT p.* FROM products p 
            INNER JOIN flash_sales fs ON p.prd_id = fs.product_id 
            WHERE p.brand_id IN (2, 35) 
            AND fs.start_time <= NOW() AND fs.end_time >= NOW() 
            ORDER BY p.prd_id ASC LIMIT 20";
} elseif ($sort === 'view_count') {
    $sql = "SELECT * FROM products WHERE brand_id IN (2, 35) ORDER BY views DESC LIMIT 20";
} else {
    $sql = "SELECT * FROM products WHERE brand_id IN (2, 35) ORDER BY prd_id ASC LIMIT 20";
}
?>
...
<h2 class="brand-title" style="width: 400px;">Chuyên Trang Samsung</h2>
